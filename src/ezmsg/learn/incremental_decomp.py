...
from ezmsg.sigproc.base import CompositeProcessor, BaseTransformerUnit, ProcessorState, BaseStatefulProcessor
...
class IncrementalDecompTransformer(
    ...
    def _initialize_processors(
        ...
        return {
            "decomp": IncrementalPCA(n_components=settings.n_components) if settings.method == "pca" else MiniBatchNMF(n_components=settings.n_components),
            "wingen": WindowTransformer(
                axis="time",
                window_dur=settings.update_interval,
                window_shift=settings.update_interval,
                zero_pad_until="none"
            ) if settings.update_interval != 0 else None,
        }
    ...
    @state.setter
    def state(self, state: dict[str, ProcessorState | IncrementalDecompSettings] | bytes | None) -> None:
    ...
    def _process(self, message: AxisArray) -> AxisArray:
        ...
        if self._procs["wingen"] is None or not hasattr(self._procs["decomp"], "components_"):
        ...
        decomp_dat = self._procs["decomp"].transform(in_dat).reshape((-1,) + self._state.template.data.shape[1:])
        return replace(self._state.template, data=decomp_dat)


class IncrementalDecomp(
    BaseTransformerUnit[IncrementalDecompSettings, AxisArray, IncrementalDecompTransformer]
):
    SETTINGS = IncrementalDecompSettings
